# KMP Publish macOS App to the App Store

This guide walks you end-to-end through creating Apple signing assets, wiring your Compose Multiplatform (Desktop) Gradle setup, adding one Fastlane lane (all logic inline), and using GitHub Actions to build & submit your macOS app to the App Store (production).
...
1. Capture screenshots in your Mac.
...
3. Name the screenshots following this convention (good practice): `[index]_APP_[DEVICE_TYPE]_[DISPLAY/GEN]_[RESOLUTION]_[INDEX].png`.
    - Example: `0_APP_DESKTOP_0.png`.
4. Place the screenshots in a localized folder inside `screenshots/`, such as `screenshots/en-GB/`.

**Optional: Generate screenshots online (free)**  
If you don’t have ready-made images, you can generate Mac screenshots with the correct sizes using [Hotpot’s MacBook template](https://hotpot.ai/templates/macbook-screenshot  ):
It’s free to use and provides exported PNGs at the accepted macOS App Store dimensions (1280×800, 1440×900, 2560×1600, 2880×1800). After exporting, name the files as shown above (e.g., `0_APP_DESKTOP_0.png`) and place them in your locale folder (e.g., `fastlane/screenshots/en-US/`).

Example Screenshot Structure:
For `en-US` localization, your `screenshots/en-US/` folder might look like this (with at least 1 screenshot up to 10):
...
```yaml
platform :mac do
     desc "Build, package & submit macOS (.pkg) to the App Store"
     lane :desktop_release do |options|
        ...
        # Submit with deliver (PRODUCTION to App Store)
        deliver(
          platform: 'osx',
          pkg: pkg_path,
...
## 9) GitHub Actions Workflow (using the composite action)
  ...
  ship_mac:
    ...
    steps:
      ...
      - name: Build & Upload to TestFlight
        uses: mifos-x-actionhub-publish-macos-on-appstore@main
        with:
